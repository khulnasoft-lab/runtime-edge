...
```sh npm2yarn
npm install @runtime-edge/node-utils
```
...
import { buildToNodeHandler } from '@runtime-edge/node-utils'
...
Otherwise, you can reuse primitives from [@runtime-edge/primitives](/packages/primitives)

```js
import { buildToNodeHandler } from '@runtime-edge/node-utils'
import * as primitives from '@runtime-edge/primitives'
